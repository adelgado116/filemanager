...
class PDF extends FPDF
{
	...
	function Header() {
		...
		// address
		$address = "Raytheon Anschuetz Panama\nCiudad del Saber, Clayton\nBuilding 225\nRep. of Panama";
		...
		$this->SetXY((45+7), $letterheadStart);
		$this->Cell(2,15, '', 'L', 0);
		$this->SetFont('Arial','',8);  //Arial normal 9
		...
		$this->Cell(100,10,'Workshop Repair',0,1,'C');  //Title
...
if ( $data['workshop_in_id'] == '' ) {

	$pdf->Cell(100,10, 'Order No.:  N/A', 0,1,'C');
	
} else {
	
	$pdf->Cell(100,10, 'Order No.:  WR'.$data['workshop_in_id'], 0,1,'C');
	//$pdf->Cell(100,10, 'Order No.:  ______', 0,1,'C');
}
...
$pdf->Cell(100,10,'Workshop Repair Worksheet',0,1,'C');
$pdf->Ln(0.3);
...
if ( $data['workshop_in_id'] == '' ) {
	...
	$pdf->Cell(100,10, 'Receipt No.:  N/A', 0,1,'L');
	
} else {
	
	$pdf->Cell(100,10, 'Receipt No.:  WR'.$data['workshop_in_id'], 0,1,'L');
	//$pdf->Cell(100,10, 'Order No.:  ______', 0,1,'L');
}

// date
$pdf->SetXY( $x_start - 8, 167 );
$pdf->SetFont('Arial','',12);  //Arial normal 16
$pdf->Cell(100,10, 'Date:  '.date('j / M / Y'), 0,1,'L');
...
$pdf->Text($left, $y, $data['ship_name']);
$pdf->Text($left, $y + 8, $data['imo']);
$pdf->Text($left, $y + 16, $data['customer']);
$pdf->Text($left, $y + 24, $data['customer_phone']);
$pdf->Text($left, $y + 32, $data['customer_email']);

$pdf->Text($right, $y, $data['EQUIP_TYPE']);
$pdf->Text($right, $y + 8, $data['MANUF_NAME']);
$pdf->Text($right, $y + 16, $data['EQUIP_MODEL']);
$pdf->Text($right, $y + 24, $data['part_no']);
$pdf->Text($right, $y + 32, $data['sn']);

$pdf->Text($left, $y + 65, $employees['emp_login']);

$pdf->SetXY( 13, $y + 38 );
$pdf->MultiCell( 184, 4, $data['notes'], 0, '' );
...
$pdf->Output( $path.$data['workshop_in_id'].'-'.date('dmY').'.pdf', 'F' );
