...
//Class_Initialize Event @2-1D91AAA0
    function clsRecordLogin($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->login = & new clsControl(ccsListBox, "login", "login", ccsText, "", CCGetRequestParam("login", $Method, NULL), $this);
            $this->login->DSType = dsTable;
            $this->login->DataSource = new clsDBhss_db();
            $this->login->ds = & $this->login->DataSource;
            $this->login->DataSource->SQL = "SELECT * \n" .
"FROM employees_tbl {SQL_Where} {SQL_OrderBy}";
            $this->login->DataSource->Order = "emp_login";
            list($this->login->BoundColumn, $this->login->TextColumn, $this->login->DBFormat) = array("emp_login", "emp_login", "");
            $this->login->DataSource->Order = "emp_login";
...
//Go to destination page @1-7970CE94
if($Redirect)
{
