...
//Connection Settings @0-69CDFD8E
...
//Initialize Common Variables @0-53DF7702
...
define("CCS_ENCRYPTION_KEY_FOR_COOKIE", '9mXr61n9a0Hp8P9L');
...
//hss_db Connection Class @-C45AFB36
class clsDBhss_db extends DB_Adapter
{
    function clsDBhss_db()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["hss_db"]);
        parent::Initialize();
...
//GenerateCaptchaCode @0-C2CA0F74
function GenerateCaptchaCode($letters, $sesVariableName, $width, $height, $length, $rot, $br, $w1, $w2, $noise) {
    ...
    for ($i = 0; $i < $length; $i++) {
        ...
        $t->Wave(2 * $w1 * (mt_rand(0, 99)/100) - $w1);
    ...
    $res->Rotate(90);
    $res->Wave(2 * $w2 * (mt_rand(0, 99)/100) - $w2);
    $res->Rotate(-90);
    $res->Broke($br, $br);
...
//CCLoginUser @0-CB4DE5DE
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBhss_db();
    $SQL = "SELECT emp_id, group_id, emp_password FROM employees_tbl WHERE emp_login=" . $db->ToSQL($login, ccsText) . " AND emp_password=MD5(" . $db->ToSQL($password, ccsText) . ")";
    $db->query($SQL);
