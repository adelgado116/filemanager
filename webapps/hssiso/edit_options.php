...
#
#	get filter parameters coming from edit_options_frame.php
#

# type of report
if ( isset( $_REQUEST['filter_report_type'] ) ) {
	
	$filter_rep_id = $_REQUEST['filter_report_type'];

	if ( $_REQUEST['filter_report_type'] == 0 ) {	
		$sql = "SELECT * FROM reports_options_list ORDER BY rep_id ASC";
	} else {
		$sql = "SELECT * FROM reports_options_list WHERE rep_id='$filter_rep_id' ORDER BY rep_opt_id ASC";
	}
} else {
	$sql = "SELECT * FROM reports_options_list ORDER BY rep_id ASC";
}
...
<table class="tableText" border="0">
	<thead>
		<tr bgcolor="#00CCFF" align="center">
			<td>REPORT TYPE</td><td>REPORT OPTION</td><td>SHOW</td>
		</tr>
	</thead>
	<!--<tfoot>
		<tr bgcolor="#00CCFF">
	...
	<tbody>
		...
		<td colspan="6" align="center">
			...
			<input type="hidden" name="filter_report_type" value="<?php echo $filter_rep_id; ?>" />
		...
		<!-- lower navigator -->
		<td colspan="6" align="center">
			<a href="edit_options.php?current=<?php echo $currentPage; ?>&filter_report_type=<?php echo $_REQUEST['filter_report_type']; ?>&page=prev">&lt;&lt; prev page</a>
			<?php echo ' [ '.$currentPage.' of '.$pagesQty.' ] '; ?>
			<a href="edit_options.php?current=<?php echo $currentPage; ?>&filter_report_type=<?php echo $_REQUEST['filter_report_type'];; ?>&page=next">next page &gt;&gt;</a>
		</td>
		</tr>
	...
	if ($_REQUEST['new_rep_option'] != "") {
		...
		*/
		...
		if ( $wrong == 0 ) {
			...
			header("Location: edit_options.php?current=".$_REQUEST['pagenumber']."&wrong=".$wrong."&new_option_again=".$_REQUEST['new_rep_option'].'&filter_report_type='.$_REQUEST['filter_report_type'] );
		}
		
	} else {
	
		# just display the same page once again with new values from database
		header('Location: edit_options.php?current='.$_REQUEST['pagenumber'].'&filter_report_type='.$_REQUEST['filter_report_type'] );
	}
}
?>
